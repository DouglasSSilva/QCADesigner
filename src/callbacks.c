...
void use_import_standard_cell_button_clicked (GtkWidget *widget, gpointer data)
  {
    if (project_options.USE_size != 5) {
      ...
      gtk_dialog_run (GTK_DIALOG (msg = gtk_message_dialog_new (GTK_WINDOW (main_window.main_window), GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
        "To import standard cells USE size needs to be 5.\nCurrent USE size it is %d.", project_options.USE_size))) ;
      gtk_widget_hide (msg) ;
      gtk_widget_destroy (msg) ;
      return;
    }

    int sc_selected = get_standard_cell(GTK_WINDOW (main_window.main_window));
    if(sc_selected == -1) return;
    char *fileName;
    ...
    if (open_project_file (completeFileName, &sel)) {
      ...
      GtkWidget *msg = NULL ;
      gtk_dialog_run (GTK_DIALOG (msg = gtk_message_dialog_new (GTK_WINDOW (main_window.main_window), GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
        "Failed to import standard cell from file \"%s\"!", completeFileName))) ;
      gtk_widget_hide (msg) ;
      gtk_widget_destroy (msg) ;
      }
...
void use_resize_button_clicked (GtkWidget *widget, gpointer data)
  {
...
void use_button_clicked (GtkWidget *widget, gpointer data)
  {
  if (project_options.SHOW_USE==FALSE) {
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(main_window.show_USE_clocking_scheme_menu_item), TRUE);
  }
  else {
...
void on_fault_simulation_start_menu_item_activate(GtkMenuItem * menuitem, gpointer user_data)
{
	  ...
	  char *output_files_path=fault_simulation_options.output_files_path;
	  char *pszFName = NULL ;
	  char *pszTempFName = NULL ;
	  char *psz = NULL ;
	  char *ind = NULL;
	    ...
	    for (n = 1 ; n<= ns ; n++)
	    {
	    	  ...
	    	  create_simulation_output_file (psz, &sim_output) ;

	    }

}  //on_fault_simulation_start_menu_item_activate

void on_fault_simulation_stop_menu_item_activate(GtkMenuItem * menuitem, gpointer user_data)
{
  //DBG_CB_HERE (fprintf (stderr, "Entering on_fault_simulation_stop_menu_item_activate\n")) ;
  //get_sim_fault_from_user (GTK_WINDOW (main_window.main_window), &fault_simulation_options) ;

}  //on_fault_simulation_stop_menu_item_activate
