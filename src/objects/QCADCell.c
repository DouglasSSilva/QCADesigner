...
static gboolean unserialize (QCADDesignObject *obj, FILE *fp)
  ...
  cell->cell_options.is_standard_cell = -1;
  ...
  while (TRUE)
    ...
    if (!bStopReading)
      ...
      if (!strcmp (pszLine, "cell_options.is_standard_cell")) {
        cell->cell_options.is_standard_cell = g_ascii_strtod (pszValue, NULL) ;
      }
      else
...
void qcad_cell_set_clock (QCADCell *cell, int iClock)
  {
  if(cell->cell_options.is_standard_cell == 1) return;
